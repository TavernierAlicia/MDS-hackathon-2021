...
func rightsPage(c *gin.Context) {
	...
	data := []*ArticleInfos{}
	db.Select(&data, "SELECT article.id, article.title, article.main_picture, article.creation_date, CONCAT(LEFT(article_content.text, 100), '...') AS text, 0.0 AS relevance FROM article JOIN article_content ON article.id = article_content.article_id GROUP BY article.id LIMIT 4")
	c.HTML(200, "rights.html", map[string]interface{}{"data": data, "keywords": keyword, "rows": len(data)})
}
...
func searchRights(c *gin.Context) {
	...
	data := []*ArticleInfos{}
	db.Select(&data, "SELECT article_content.article_id AS id, article.title, article.main_picture, article.creation_date, CONCAT(LEFT(article_content.text, 100), '...') AS text, SUM(MATCH (article_content.low_title) AGAINST('"+keywords+"*') + MATCH (article_content.text) AGAINST('"+keywords+"*') + MATCH (article.title) AGAINST ('"+keywords+"*')) as relevance FROM article_content JOIN article ON article_content.id = article.id WHERE MATCH (article_content.low_title) AGAINST('"+keywords+"*' IN BOOLEAN MODE) OR MATCH (article_content.text) AGAINST('"+keywords+"*' IN BOOLEAN MODE) OR MATCH (article.title) AGAINST ('"+keywords+"*' IN BOOLEAN MODE) GROUP BY article_id ORDER BY relevance")
	c.HTML(200, "rights.html", map[string]interface{}{"data": data, "keywords": keywords, "rows": len(data)})
...
func searchAll(c *gin.Context) {
	...
	Block{
		Try: func() {
			...
			allnil = func() int {
				if len(houses.Records) == 0 && len(articles) == 0 && len(faq) == 0 {
					return 1
				} else {
					return 0
				}
			...
			c.HTML(200, "searchResults.html", map[string]interface{}{"houses": houses.Records, "articles": articles, "faq": faq, "keywords": keywords, "allnil": allnil, "Hrows": len(houses.Records), "Arows": len(articles), "Frows": len(faq)})
		},
		Catch: func(e Exception) {
			log.Error("Caught %v\n " + fmt.Sprintf("%v", e))
			errorPage(c)
